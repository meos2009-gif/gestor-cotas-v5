...
export default function Contabilidade() {
  ...
  const [category, setCategory] = useState("seguros");
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex space-x-4 mb-6">
        ...
        <select value={ano} onChange={(e) => setAno(e.target.value)} className="border p-2 rounded bg-white text-black">
          <option value={2024}>2024</option>
          <option value={2025}>2025</option>
          <option value={2026}>2026</option>
        </select>

        <select value={categoriaFiltro} onChange={(e) => setCategoriaFiltro(e.target.value)} className="border p-2 rounded bg-white text-black">
      ...
      <div className="flex space-x-4 mb-6">
        ...
        <button onClick={() => { resetForm(); setType("saida"); setModalOpen(true); }} className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded">
          Adicionar Saída
        </button>
      </div>
      ...
      <div className="bg-primary text-white rounded-lg shadow-md overflow-hidden overflow-x-auto">
        <table className="w-full text-sm min-w-[600px]">
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan="7" className="p-4 text-center text-white">A carregar...</td>
              </tr>
            ) : movements.length === 0 ? (
              <tr>
                <td colSpan="7" className="p-4 text-center text-white">Sem movimentos registados.</td>
              </tr>
            ) : (
              movements.map((m) => (
                <tr key={m.id} className="border-t border-gray-700">
                  <td className="p-3 text-white">{m.date}</td>
                  <td className="p-3 text-white capitalize">{m.type}</td>
                  <td className="p-3 text-white capitalize">{m.category}</td>
                  <td className="p-3 text-white">{m.description || "-"}</td>
                  <td className="p-3 text-white">{m.obs || "-"}</td>
                  <td className={`p-3 text-right font-semibold ${m.type === "entrada" ? "text-green-400" : "text-red-400"}`}>
                    {m.type === "entrada" ? "+" : "-"}
                    {Number(m.value).toFixed(2)} €
                  </td>
                  <td className="p-3 text-right">
                    <button onClick={() => openEditModal(m)} className="bg-secondary text-primary px-3 py-1 rounded hover:bg-white hover:text-primary transition">
                      Editar
                    </button>
                  </td>
                </tr>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-80">
            ...
            <input type="text" className="w-full border p-2 rounded mb-4" value={obs} onChange={(e) => setObs(e.target.value)} />
